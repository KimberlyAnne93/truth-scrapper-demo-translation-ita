...
python early:
    ...
    renpy.register_sl_displayable("controller_bar", ControllerBar, "slider", 0, replaces=True,
        pass_context=True
    ...
    renpy.register_sl_displayable("controller_vbar", controller_sl2vbar, "vslider", 0, replaces=True,
        pass_context=True,
        ).add_property("hovered"
        ).add_property("unhovered"
        ).add_property("update_interval"
    ...
    class FocusDisplayable(renpy.Displayable):
        ...
        def get_displayable_name(self):
            ...
            disp_kind = renpy.display.focus.get_mouse() 
            
            if disp_kind is None:
                disp_kind = "default"
            
            if pygame.mouse.get_pressed()[0]:
                disp_kind = "pressed_" + disp_kind 
            ...
            if disp_kind.startswith("pressed_") and ("pressed_default" in self.displayables): 
                
                disp_kind = "pressed_default"
            elif disp_kind.startswith("pressed_") and (disp_kind[8:] in self.displayables): 
    ...
    class StickEvent(Null):
        ...
        def calculate_new_stick_position(self):
            ...
            self.angle = (math.degrees(math.atan2(up_down_pct, left_right_pct))+90) % 360 
